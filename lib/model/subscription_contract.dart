...
class SubscriptionContract {
  ...
  @override
  String toString() =>
      'SubscriptionContract[id=$id, isActive=$isActive, createdOnUtc=$createdOnUtc, subject=$subject, subscriber=$subscriber, activateBeforeUtc=$activateBeforeUtc, expiresOnUtc=$expiresOnUtc]';
  ...
  static SubscriptionContract? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "SubscriptionContract[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "SubscriptionContract[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SubscriptionContract> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <SubscriptionContract>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SubscriptionContract.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SubscriptionContract> mapFromJson(dynamic json) {
    final map = <String, SubscriptionContract>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SubscriptionContract.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SubscriptionContract-objects as value to a dart map
  static Map<String, List<SubscriptionContract>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<SubscriptionContract>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SubscriptionContract.listFromJson(
          entry.value,
          growable: growable,
        );
      }
