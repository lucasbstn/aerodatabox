...
class AirportFlightContractMovement {
  /// Returns a new [AirportFlightContractMovement] instance.
  ...
  bool operator ==(Object other) =>
      ...
      other is AirportFlightContractMovement &&
  ...
  @override
  String toString() =>
      'AirportFlightContractMovement[airport=$airport, quality=$quality, scheduledTime=$scheduledTime, revisedTime=$revisedTime, runwayTime=$runwayTime, terminal=$terminal, checkInDesk=$checkInDesk, gate=$gate, baggageBelt=$baggageBelt, runway=$runway]';
  ...
  /// Returns a new [AirportFlightContractMovement] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AirportFlightContractMovement? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "AirportFlightContractMovement[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "AirportFlightContractMovement[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return AirportFlightContractMovement(
  ...
  static List<AirportFlightContractMovement> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <AirportFlightContractMovement>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AirportFlightContractMovement.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AirportFlightContractMovement> mapFromJson(dynamic json) {
    final map = <String, AirportFlightContractMovement>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AirportFlightContractMovement.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AirportFlightContractMovement-objects as value to a dart map
  static Map<String, List<AirportFlightContractMovement>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<AirportFlightContractMovement>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = AirportFlightContractMovement.listFromJson(
          entry.value,
          growable: growable,
        );
      }
