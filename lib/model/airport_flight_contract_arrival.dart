...
class AirportFlightContractArrival {
  /// Returns a new [AirportFlightContractArrival] instance.
  ...
  bool operator ==(Object other) =>
      ...
      other is AirportFlightContractArrival &&
  ...
  @override
  String toString() =>
      'AirportFlightContractArrival[airport=$airport, quality=$quality, scheduledTime=$scheduledTime, revisedTime=$revisedTime, runwayTime=$runwayTime, terminal=$terminal, checkInDesk=$checkInDesk, gate=$gate, baggageBelt=$baggageBelt, runway=$runway]';
  ...
  /// Returns a new [AirportFlightContractArrival] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AirportFlightContractArrival? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "AirportFlightContractArrival[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "AirportFlightContractArrival[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return AirportFlightContractArrival(
  ...
  static List<AirportFlightContractArrival> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <AirportFlightContractArrival>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AirportFlightContractArrival.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AirportFlightContractArrival> mapFromJson(dynamic json) {
    final map = <String, AirportFlightContractArrival>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AirportFlightContractArrival.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AirportFlightContractArrival-objects as value to a dart map
  static Map<String, List<AirportFlightContractArrival>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<AirportFlightContractArrival>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = AirportFlightContractArrival.listFromJson(
          entry.value,
          growable: growable,
        );
      }
